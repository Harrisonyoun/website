...
```{mermaid}
%%| fig-cap: "A Connected Graph Formed by Worker Mobility"
%%| fig-alt: "A cleaner diagram showing a main network where worker moves link Firms A, B, C, and D, and a separate, isolated network of Firms X and Y."
graph LR
    style graph fill:#fff,stroke:#333,stroke-width:1px

    subgraph Main Connected Set
        direction LR
        A(Firm A) -->|Alice moves| B(Firm B)
        B -->|Bob moves| C(Firm C)
        C -->|Charles moves| D(Firm D)
    end

    subgraph Isolated Set
        direction LR
        X(Firm X) -->|Eve moves| Y(Firm Y)
    end

    style A fill:#bdd7e7,stroke:#2980b9,stroke-width:1.5px
    style B fill:#bdd7e7,stroke:#2980b9,stroke-width:1.5px
    style C fill:#bdd7e7,stroke:#2980b9,stroke-width:1.5px
    style D fill:#bdd7e7,stroke:#2980b9,stroke-width:1.5px

    style X fill:#fde0dd,stroke:#c92a2a,stroke-width:1.5px
    style Y fill:#fde0dd,stroke:#c92a2a,stroke-width:1.5px
...
For instance, if Alice moves from Firm A to B, and Charles moves from Firm B to C, we can now link all three firms and compare the relative pay premiums of A, B, and C. As long as these chains of moves are sufficiently widespread throughout the economy, the effects for most firms and workers can be identified. In the French dataset analyzed by AKM, over 88% of the labor force was part of this single connected network.

---

## Estimation Strategy: Solving a Massive Computational Problem
