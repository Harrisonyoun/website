---
title: "The Counterfactual Turn: Causal Inference Between Hume and the Credibility Revolution"
...
categories: [Economic Methodology, Philosophy of Science, Causal Inference, Econometrics, Scientific Knowledge]
...
## Hume's Challenge, in One Picture
...
But Hume also let slip a second, more fertile definition, almost as an aside: a cause is something such that, *if it had not occurred, the effect would not have occurred either*. This counterfactual formulation is the seed of the modern theory of causation — and, as we will see, the seed of modern econometrics. The irony is exact. Economics inherits both halves of Hume. Its **methods** are strictly Humean: everything is built up from observable regularities, the joint distribution of what we actually measure. Its **targets** are anti-Humean: the counterfactual dependence that Hume said was never in the data to begin with.
...
Take a binary treatment $D \in \{0,1\}$ — say, $D=1$ if a worker enrolls in the training program, $D=0$ if not. For each individual $i$, the framework posits *two* potential outcomes:
...
requires *both* numbers for the same person at the same time. We never have both. This is **Holland's fundamental problem of causal inference**: individual causal effects are, in principle, unobservable. Not hard to measure — *logically* unobservable.
...
**Lewis's nearest possible world.** The philosopher David Lewis analyzed "if $A$ had happened, $C$ would have happened" as a claim about similarity among possible worlds: take the world most similar to ours in which $A$ holds, and check whether $C$ holds there. Applied to our worker, $Y_i(0)$ is her earnings in the *closest* possible world in which she did not enroll — closest meaning: change as little as possible, then see what her earnings are. The picture is vivid and intuitively right. Its weakness is that "closest" is vague. Which similarities count? If she hadn't enrolled, would the labor market be the same, her motivation the same, her family circumstances the same? The truth value of the counterfactual wobbles with how we resolve "similar."
...
- **Unconfoundedness:** $\{Y(0), Y(1)\} \perp D \mid X$ — within cells defined by covariates $X$, who got treated is unrelated to their potential outcomes;
- **Overlap:** $0 < P(D=1 \mid X=x) < 1$ — both treated and untreated units exist in every cell;
...
**Rung 3 is not.** Genuinely unit-level counterfactual questions — the effect of treatment on a treated individual, mediation ("how much of the effect runs through this channel?"), "the probability that the drug, not luck, caused *this* recovery" — depend on the **joint** distribution of $(Y_i(0), Y_i(1))$ for the same unit. And the joint distribution is *not* determined by the two marginals. A primitive $Y_i(d)$, posited one potential outcome at a time, simply does not contain the information. To get it, you must add structure — a dependence assumption, or a full structural model à la Pearl — and that structure resurrects exactly the truth-maker question modal instrumentalism had deferred. The deferred bill comes due, and it comes due precisely at rung 3.
...
If identifying assumptions are untestable bridges, the natural question is how to be honest about leaning on them. Charles Manski's answer organizes the modern methodological conscience: the **Law of Decreasing Credibility** — *the credibility of inference decreases with the strength of the assumptions maintained.* Strong assumptions buy sharp, point-identified answers at the cost of believability. Weak assumptions buy believability at the cost of sharpness.
...
Reassemble the four parts. Logical positivism (Part 1) asked what makes a claim *meaningful*. Haavelmo's probability approach (Part 2) asked what makes a model *identified*. Popper (Part 3) asked what makes a claim *scientific* — answerable: that it be put at risk. Causal inference (Part 4) asks the sharpest version of all three at once: what makes a claim about *what would have happened otherwise* something a non-experimental science may responsibly assert.
