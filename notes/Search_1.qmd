...
Our last discussion on the AKM model gave us a powerful lens to separate a worker's portable skill (*who* they are) from a firm's pay policy (*where* they work). But this opens a new, fascinating puzzle: **Why do certain workers end up at certain firms in the first place?**

It's clearly not a random lottery. The most sought-after employees often work at the most prestigious companies. This note provides a guided tour through the economic theories that explain this sorting process. We'll start with a "perfect world" model to build our intuition, then add the complications of real-world job searching.

Throughout, we will present the formal economic models and mathematics exactly as they appear in the academic literature—no simplification of the core concepts. The goal is to build a bridge between the intuitive questions and the formal answers.
...
The frictionless model is elegant, but the real world is messy. People have to search for jobs, send out resumes, and go to interviews. The **Shimer and Smith (2000)** model brings these crucial **search frictions** into the picture.
...
$$
> **This is a crucial result.** Notice the $\alpha f(x,y)$ term. The wage isn't just a sum of a worker part and a firm part. It explicitly depends on the *interaction* between the two. This is the "smoking gun" that shows how sorting models fundamentally conflict with the simple additive structure of the AKM model.
...
This is great in theory, but how can we find evidence of these sorting patterns in messy, real-world data? Economists have developed some clever strategies to detect and measure sorting.
...
---
title: "Who Works Where? The Economics of Labor Market Sorting"
author: 'Harrison Youn<br><span style="font-size: 0.8em; color: #6c757d;">AKM 2</span>'
date: "2025-08-11"
description: "The best workers often land at the best firms. But why? We explore the economic forces of sorting, from perfect matches in ideal markets to the messy reality of job searching."
...
## The Big Question: Who Ends Up Working Where?

The AKM model gives us a brilliant tool to separate a worker's value (*who* they are) from a firm's pay premium (*where* they work). But it opens up an even deeper question: **What determines how workers and firms match up in the first place?**

It's clearly not random. We often see the most talented people working at the most successful companies. This tendency for "likes to attract" is called **positive assortative matching (PAM)**. But is this always the case? Could it ever be more efficient for opposites to attract?

This note dives into the economics of sorting. We’ll explore the beautiful logic of a perfect, frictionless job market and then see how that logic changes when we add the frictions of real-world job searching.

---

## Imagine a Perfect Job Market: The Becker Model

Let's start with a thought experiment, based on Nobel laureate Gary Becker's work. Imagine a job market with no search costs or uncertainty.

#### The Setup
There's a spectrum of workers, from low-skill to high-skill (let's call their type '$x$'), and a spectrum of firms, from low-productivity to high-productivity (type '$y$'). When a worker $x$ and a firm $y$ team up, they produce a certain amount of value, or output, which we can write as a function $f(x,y)$.

The key assumption is **transferable utility**: the output they create can be split between them in any way. The firm's share is its profit, and the worker's share is their wage.

#### What is a "Stable" Match?
In this perfect market, a set of matches is considered **stable** if no one has an incentive to break things up. This means two simple things:
1.  Everyone is better off in their match than they would be alone.
2.  More importantly, there's no rogue worker and firm who could ditch their current partners, form a new match, and *both* be better off from the deal. They can't form a "blocking pair."

This leads to a powerful conclusion: any stable matching *must* be the one that creates the most possible output for the entire economy. Why? Because if there were a different set of matches that produced more total output, some pairs could swap partners to realize that gain, making the original setup unstable. So, stability implies maximum efficiency.

---

## Likes Attract vs. Opposites Attract: The Power of Complementarity

So, what determines whether the best match with the best, or if opposites attract? It all comes down to one crucial concept: **complementarity**.

#### Supermodularity: When 1 + 1 > 2
Think of a brilliant software engineer and a company with massive computing power. The great engineer makes the company's technology shine, and the advanced tech lets the engineer build things they couldn't otherwise. They are **complements**. The boost each gives the other is more than the sum of their parts.

In economics, this is called **supermodularity**. Mathematically, it means the cross-partial derivative of the production function is positive ($f_{xy} > 0$). It captures the idea that a worker's skills are more valuable when paired with a better firm.

#### Submodularity: When It's Better to Diversify
Now, imagine a creative agency that needs both a charismatic salesperson and a meticulous project manager. Pairing two star salespeople might lead to chaos, while pairing two project managers might lead to inaction. The most productive team pairs the salesperson with the manager. Their skills are, in a sense, **substitutes**.

This is called **submodularity** ($f_{xy} < 0$). Here, the value of a high-skill worker is actually greater when paired with a firm that has a different, "lower" type of strength.

#### Becker's Famous Result
This simple distinction leads to a foundational theorem in matching theory:
> - If worker skill and firm productivity are **complements** ($f_{xy} > 0$), the only stable outcome is **Positive Assortative Matching (PAM)**. The best workers go to the best firms, all the way down the line.
> - If they are **substitutes** ($f_{xy} < 0$), the stable outcome is **Negative Assortative Matching (NAM)**. The best workers match with the "worst" firms, and vice versa, to balance out skills.

Under PAM, wage inequality naturally gets stretched. The top workers get a "double whammy": their own high skill *plus* the amplified productivity from their top-tier firm, leading to extremely high wages.

---

## Entering the Real World: Job Searching and Frictions

Of course, the real job market isn't a perfect, one-shot matching game. People have to search, interviews happen, and not every worker meets every firm. This is where **search frictions** come in, as modeled by Robert Shimer and Lones Smith.

#### The Problem of "Holding Out"
Search frictions introduce a new dilemma: the **option value of waiting**.

Imagine a pretty good worker meets a pretty good firm. In Becker's world, they'd match. But in the real world, the worker might think, "This is fine, but if I just wait a bit longer, maybe I'll get an offer from a superstar firm." The firm might think the same thing. If both decide to hold out for a better option, a perfectly good match might never happen.

This means that simple complementarity ($f_{xy} > 0$) is **no longer enough** to guarantee that the best will match with the best. The temptation to wait for a perfect partner can cause the sorting pattern to break down, especially for those in the middle of the skill distribution.

#### The Stronger Condition: Log-Supermodularity
Shimer and Smith showed that for PAM to survive in a world with search frictions, you need a much stronger condition called **log-supermodularity**.

The intuition is that the *percentage gain* a worker gets from moving to a better firm has to be greater if they are a high-skill worker. This ensures that the pull of a better match is so powerful that it overwhelms the temptation to wait, keeping the "likes attract" sorting pattern intact.

> In a frictional world, sorting is harder. It requires not just that good workers and good firms are complements, but that they are *so strongly* complementary that they'd rather take a good-enough match now than hold out for a perfect one that may never come.

---

## How Sorting Theory Challenges the AKM Model

This entire theory of matching creates some serious headaches for the simple, additive AKM model. There are two big problems.

#### 1. Wages Aren't Just `Worker + Firm`
In a sorting world, a wage isn't just the sum of a worker effect and a firm effect. It's the result of a bargain over the **match surplus**. This means the wage inherently includes a **match-specific interaction term**. The AKM model has no place for this; it forces the match-specific part into the error term, which can bias the results and make it seem like sorting is weaker than it really is.

#### 2. The "Disconnected Market" Problem
The AKM model can only separate worker and firm effects because it sees workers move between different companies. But what if sorting were perfect?

Imagine the labor market split into a "Premier League" and a "Second Division," with no worker ever moving between them. If you only have data from this market, you could never know if the Premier League's high wages are because its workers are amazing or because its firms are generous. You could add $100,000 to every firm premium in the Premier League and subtract it from every worker effect, and the wages would look exactly the same.

The only reason AKM works at all is because the real world is messy. Sorting isn't perfect, and some workers *do* move between different "leagues," creating a **connected set** that allows us to statistically tell the effects apart.

---

## Finding the Evidence: How Economists Measure Sorting

Given these challenges, economists have devised clever ways to find evidence of sorting.

-   **The HLM Rank Method**: Named after Hagedorn, Law, and Manovskii, this is like solving a giant puzzle. You first rank workers within each firm by their wage. Then, you use workers who move between firms as "bridges" to link these individual firm rankings into one economy-wide skill ladder. You can do the same for firms, ultimately allowing you to see the sorting pattern and even estimate the degree of complementarity.
-   **Poaching Flows**: Another method, championed by Isaac Sorkin, is to let workers "vote with their feet." If we consistently see workers leaving Firm A to go to Firm B, we can infer that B is a higher-ranked firm. By mapping out all these "poaching flows," we can create a firm hierarchy based on revealed preference.

These methods generally confirm that PAM is a real feature of our labor market, though the strength of complementarity is still a subject of intense debate.

---

## Conclusion: Why Sorting Matters

The theory of assortative matching profoundly enriches our understanding of the labor market. It tells us that the simple story of worker effects and firm effects is incomplete.

1.  **It Drives Inequality**: Positive sorting naturally pulls wages apart. It creates a "superstar" economy where the best-matched pairs generate huge value and earn huge rewards, widening the gap between the top and the bottom.
2.  **It Affects Productivity**: Frictions that prevent efficient sorting are not just unfair; they are inefficient. A high-skilled worker stuck at a low-productivity firm represents a loss of potential output for the whole economy.
3.  **It Re-frames AKM**: It teaches us to be cautious about interpreting AKM results. A small correlation between worker and firm effects doesn't necessarily mean sorting is weak. It might just mean the simple additive model is failing to capture the crucial role of the **match itself**.

Ultimately, understanding who works where—and why—is fundamental to understanding some of the biggest economic questions of our time, from wage inequality to national productivity.
